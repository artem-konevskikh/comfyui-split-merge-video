...
# Try to import ComfyUI's folder_paths, fallback to default paths if not available
try:
    import sys
    ...
    class FolderPaths:
        ...
        def get_temp_directory():
            return os.path.join(os.path.dirname(os.path.dirname(__file__)), 'temp')
    ...
    def split_video(
        ...
        for i, start in enumerate(start_times):
            ...
            output_path = os.path.join(
                output_dir, f"{filename}_segment_{i:03d}_{start:05.1f}-{end:05.1f}.mp4"
            )
...
class VideoMergerNode:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "segments_dir": ("STRING", {"default": "", "multiline": False}),
                "fade_duration": (
                    "FLOAT",
    ...
    def merge_videos(
        ...
        output_file = os.path.join(
            folder_paths.get_output_directory(), "merged_video.mp4"
        )
